...
async def create_access_token(username: str, user_id: int, email: str, expires_delta: timedelta):
...
async def login(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], db: AsyncSession = Depends(get_db)):
    ...
    token = await create_access_token(user.username, user.id, user.email,
